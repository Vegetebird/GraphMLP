...
class Fusion_3dhp(data.Dataset):
    ...
    def prepare_data(self, dataset, folder_list):
        ...
        keypoints_pth = self.root_path + 'data_2d_' + self.data_type + '.npz'
        
        keypoints = np.load(keypoints_pth, allow_pickle=True)
        keypoints_GT = np.load(keypoints_pth,allow_pickle=True)
        
        self.kps_left, self.kps_right = [4,5,6,11,12,13], [1,2,3,14,15,16]
        self.joints_left, self.joints_right = [4,5,6,11,12,13], [1,2,3,14,15,16]
        ...
        for subject in keypoints_GT.keys():
            for action in keypoints_GT[subject]:
                for cam_idx, item in enumerate(keypoints_GT[subject][action]):
                    ...
                    if self.crop_uv == 0:
                        kps[..., :2] = normalize_screen_coordinates(kps[..., :2], w=res_w, h=res_h)
                        kps_GT[..., :2] = normalize_screen_coordinates(kps_GT[..., :2], w=res_w, h=res_h)
